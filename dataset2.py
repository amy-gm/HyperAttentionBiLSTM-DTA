...
def collate_fn(batch_data):
    ...
    affinity_new = torch.zeros(N, dtype=torch.long)
    for i,pair in enumerate(batch_data):
        # _, _, compoundstr, proteinstr, label = pair.strip().split()
        pair = pair.strip().split()
        compoundstr, proteinstr, affinity = pair[-3], pair[-2], pair[-1]
        ...
        # affinity，亲和力值
        affinity_new[i] = np.int(affinity)
    # return (drug_ids,protein_ids, compound_new, protein_new, labels_new)
    return (compound_new, protein_new, affinity_new)
